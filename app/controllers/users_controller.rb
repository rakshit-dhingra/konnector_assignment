class UsersController < ApplicationController
  ...
  def filter
    ...
    conditions = campaign_names.map do |name|
      "JSON_SEARCH(campaigns_list, 'one', ?, NULL, '$[*].campaign_name') IS NOT NULL"
    end.join(" OR ")

    query_params = campaign_names.map { |name| [name] }.flatten
    @users = User.where(conditions, *query_params)
    render json: @users
  end
